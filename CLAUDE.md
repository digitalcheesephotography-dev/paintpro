...
- `room.photos` on a job holds only `[{ id }]` references — NO base64. A photo tagged for a reel carries `{ id, tag }` where tag is `'before'` or `'after'` (see 7.12); untagged photos stay `{ id }` so nothing grew for existing jobs. `getJobSnapshot()` strips any `.data` so localStorage never fills and the synced `ingersoll_jobs_v1` doc stays tiny (was the two critical data-loss bugs).
- `addRoomPhotoFiles()` compresses (`REF_PHOTO_PX` 1400px / `REF_PHOTO_Q` 0.72 - raised from 550/0.5 so a reference photo is also usable as reel footage on a 1080-wide canvas with room to crop in; AI-scan photos in `addAIPhotos` stay small because every one rides in an API request) → `photoPut` → pushes `{id}`. `renderRoomPhotoStrip()` is async: loads each blob from IndexedDB; a reference with no local blob renders an "other device" placeholder. `viewRoomPhoto(id)` fetches from IndexedDB for the lightbox.
...
### 7.12 Reel Kit — job photos out to CapCut — built
- **CapCut has no API.** Nothing can drive its timeline from here, and none was attempted. What CapCut does accept is a batch of images from the Android share sheet, imported in filename order. So the app's job is curation, and all of it works with no signal.
- **Tagging:** each room-photo thumbnail carries a corner badge (`cycleRoomPhotoTag`) that cycles blank → **B** (before, blue) → **A** (after, green) → blank. The tag lives on the photo reference (`room.photos[].tag`) and rides through `getJobSnapshot()`.
- **Order:** `buildReelSequence()` walks rooms in order; inside a room befores lead and afters follow, so each area reads as a pair. Untagged photos keep capture order, which is also the zero-setup fallback for a job with no tagging at all.
- **Handoff:** ☰ MENU → **🎬 Make a Reel** (`openReelKit()`) lists the sequence, then `navigator.share({files})` hands the photos to the Android share sheet, where CapCut is one of the targets. Files are named `01_BEFORE_Kitchen.jpg`, `02_AFTER_Kitchen.jpg` … so the import lands in that order. Capped at `REEL_MAX_FILES` (20) — beyond that the share sheet chokes. `_reelFile()` reads the local IndexedDB blob, falling back to a Storage download for a photo taken on the other phone; one that can't be resolved is skipped and counted in a toast.
...
**REGRESSION BENCHMARK — Robin Caster exterior.** Use this to check the exterior math after any change to `calcExt()`, the section sums, or the surface toggles. Straight off James's paper sheet, with **all surface toggles OFF** (which is how he works):
...
### Reel Kit (CapCut handoff)
`cycleRoomPhotoTag`, `buildReelSequence`, `openReelKit`, `buildReelCaption`, `buildReelBeats`, `_reelFile`, `_reelTown`, `_reelSlug`, `_reelTime`, `_reelFallback` (tag job photos before/after, then share them in order into CapCut via the Android share sheet)
...
- **⚠️ Secrets and the repo.** Two hard-coded values were taken out on Aug 31 2026. `PRINT_HUB_TOPIC` (an ntfy.sh topic is a bearer token — anyone reading this public repo could push print jobs to James's home printer) is **blanked** in both `PaintPro-ZFold.html` and `print_hub.py`; the Print Hub UI was already retired so nothing changed functionally. `LISA_EMAIL` was **not** dead code — it drove the BCC on "Email to Client" — so instead of deleting it, it became `ingersoll_bcc_email_v1`, set in Settings → 📧 Email Client a Copy and in `SYNC_KEYS` so a reinstall doesn't silently drop the blind copy. **Both values remain in git history and should be considered burned:** if the print hub is revived, generate a NEW topic and keep it out of version control.
