...
### Railings, treads & shot rounding
`shotFt`, `addRailSection`, `removeRailSection`, `updateRailSection`, `renderRailSections`, `updateRoomTreads`, `armRailShot`, `_railId`, `_seedSectionCounters` (the last one reseeds the soffit/porch/deck id counters after a job loads - they reset to 0 on reload, so "+ Add Section" on a loaded job used to hand out an id already in use and edits landed on the wrong row)
